...
"""
#<script src="{{ url_for('static', filename='js/current-chord-highlighting.js') }}"></script>
...
def teardown_db(exception):
    close_database(DATABASE_LYRICS, exception)
    close_database(DATABASE_UE, exception)
...
# Function to create a safe filename from a song title
def create_safe_filename(track_name, artist_name):
    """Create a safe filename from track and artist name"""
    # Remove invalid filename characters
    safe_name = re.sub(r'[\\/*?:"<>|]', "", f"{artist_name} - {track_name}")
    # Replace spaces with underscores and limit length
    safe_name = safe_name.replace(" ", "_")[:100]
    return safe_name + ".txt"

# Function to log LRCLIB lyrics
def log_lrclib_lyrics(track_id, track_name, artist_name, lyrics_data):
    """Log LRCLIB lyrics to a file in db_lrc directory"""
    # Create db_lrc directory if it doesn't exist
    Path("db_lrc").mkdir(exist_ok=True)
    
    # Create filename based on track and artist
    filename = create_safe_filename(track_name, artist_name)
    filepath = os.path.join("db_lrc", filename)
    ...
    try:
        ...
        print(f"LRCLIB lyrics logged to {filepath}")
        return True
    except Exception as e:
        print(f"Error logging LRCLIB lyrics: {e}")
        return False

# Function to log Ultimate Guitar tabs
def log_ultimate_guitar_tab(track_id, track_name, artist_name, source_code, source_link, found=False):
    """Log Ultimate Guitar tab to a file in db_ug directory"""
    # Create db_ug directory if it doesn't exist
    Path("db_ug").mkdir(exist_ok=True)
    
    # Create filename based on track and artist
    filename = create_safe_filename(track_name, artist_name)
    filepath = os.path.join("db_ug", filename)
    ...
    log_data = {
        ...
        "found": found,
        # Only include a snippet of the source to avoid storing too much data
        "source_snippet": source_code[:500] if source_code else None
    }
    
    try:
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(log_data, f, indent=2, ensure_ascii=False)
        print(f"Ultimate Guitar tab logged to {filepath}")
        return True
    except Exception as e:
        print(f"Error logging Ultimate Guitar tab: {e}")
        return False
...
def safe_get_current_playback(max_retries=2):
    """Safely get current playback with better error handling and retries"""
    retry_count = 0
    while retry_count < max_retries:
        try:
            token_info = refresh_token()
            if token_info == 0:
                print(format_debug("ERROR: No token info available"))
                return None
                
            spotify = spotipy.Spotify(auth=token_info['access_token'])
            
            # Try explicit playback request
            current_playback = spotify.current_playback()
            if current_playback is not None:
                return current_playback
            
            # Fallback to current_user_playing_track
            playing_track = spotify.current_user_playing_track()
            if playing_track is not None:
                return playing_track
                
            # If nothing is playing, return a simple empty state
            return {
                'item': None,
                'is_playing': False,
                'progress_ms': 0,
            ...
            print(format_debug(f"Error in safe_get_current_playback (attempt {retry_count+1}): {e}"))
            retry_count += 1
...
def api_track_data():
    """HTTP API endpoint for track data (fallback if WebSockets fail)"""
    try:
        # Get dynamic data
        dynamic_data = getTrackDynamicData()
        
        # Get static data (using 'left' as default alignment)
        static_data = getTrackStaticData("left")
        
        # Combine data
        combined_data = {**dynamic_data, **static_data}
        ...
        return json.dumps({
            'error': str(e),
            'track_name': 'Error',
            'artist_name': 'Error fetching data'
        })

@app.route('/http-fallback')
def http_fallback():
    """HTTP fallback page if WebSockets are not working"""
    return render_template('http_fallback.html')

@app.route('/api/player/play-pause', methods=['POST'])
def api_play_pause():
    """HTTP API endpoint for play/pause control"""
    try:
        token_info = refresh_token()
        if token_info == 0:
        ...
        spotify = spotipy.Spotify(auth=token_info['access_token'])
        current_track = safe_get_current_playback()
        
        if current_track is None:
            return json.dumps({'error': 'No active playback'})
        ...
        return json.dumps({'success': True, 'action': 'pause' if is_playing else 'play'})
...
def api_next_track():
    """HTTP API endpoint for next track control"""
    try:
        token_info = refresh_token()
        if token_info == 0:
            return json.dumps({'error': 'Not logged in'})
...
def api_previous_track():
    """HTTP API endpoint for previous track control"""
    try:
        token_info = refresh_token()
        if token_info == 0:
            return json.dumps({'error': 'Not logged in'})
...
@app.route('/callback')
def callback():
    # Clear previous session data but don't log out
    # This ensures we get a fresh token without removing other session data
...
@socketio.on('connect')
def handleConnect():
    print(format_debug('WebSocket: Client (JavaScript) connected to Server (Python)'))
    # Debug current spotify state
    debug_spotify_connection()
    
@socketio.on('disconnect')
def handleDisconnect():
    print(format_debug('WebSocket: Client disconnected from Server'))
    
@socketio.on('trackDynamicDataRequest')
def handleDynamicDataRequest():
    print(format_debug('trackDynamicDataRequest received'))
    data = getTrackDynamicData()
    print(format_debug(f'Sending dynamic data: {json.dumps(data, indent=2)}'))
    emit('trackDynamicDataResponse', data)
    
@socketio.on('trackStaticDataRequest')
def handleStaticDataRequest(align="left"):
    print(format_debug(f'trackStaticDataRequest received with align={align}'))
    data = getTrackStaticData(align)
    # Only print a summary to avoid overwhelming the terminal
    summary = {
        'track_name': data.get('track_name'),
        'artist_name': data.get('artist_name'),
        'spotify_error': data.get('spotify_error'),
        'found_lyrics': data.get('found_musixmatch_lyrics'),
        'lyrics_synced': data.get('musixmatch_lyrics_is_linesynced')
    }
    print(format_debug(f'Sending static data summary: {json.dumps(summary, indent=2)}'))
    emit('trackStaticDataResponse', data)

def debug_spotify_connection():
    """Debug function to check Spotify connection"""
    try:
        token_info = refresh_token()
        if token_info == 0:
            print(format_debug("No token info available"))
            return False
        ...
        print(format_debug(f"Connected to Spotify as {user_profile['display_name']}"))
        
        # Check if a track is playing
        try:
            current_playback = spotify.current_playback()
            if current_playback is None:
                print(format_debug("No active playback found"))
                return False
                
            print(format_debug(f"Currently playing: {current_playback['item']['name']} by {current_playback['item']['artists'][0]['name']}"))
            print(format_debug(f"Track ID: {current_playback['item']['id']}"))
            print(format_debug(f"Is playing: {current_playback['is_playing']}"))
            return True
        except Exception as e:
            print(format_debug(f"Error getting current playback: {e}"))
            return False
            
    except Exception as e:
        print(format_debug(f"Spotify connection error: {e}"))
        return False
...
def getTrackDynamicData():    
    """Get dynamic data for the current track"""
    current_track = safe_get_current_playback()
    
    if current_track is None or 'item' not in current_track or current_track['item'] is None:
        print(format_debug("No valid track found in getTrackDynamicData"))
        return {
            'track_id': "0",
            'progress_ms': 0,
            'current_time': "0:00",
    ...
    try:
        ...
        is_playing = str(current_track.get('is_playing', False))
        
        # Handle potential missing data for device
        if 'device' in current_track and current_track['device'] is not None:
            current_volume = current_track['device'].get('volume_percent', 50)
        else:
            current_volume = 50
            
        current_shuffle_state = current_track.get('shuffle_state', False)
        current_repeat_state = current_track.get('repeat_state', 'off')
        ...
        print(format_debug(f"Error processing track data in getTrackDynamicData: {e}"))
        return {
            'track_id': "0",
            'progress_ms': 0,
            'current_time': "0:00",
...
def getTrackStaticData(align):        
    ...
    token_info = refresh_token()
    if token_info == 0:
        print(format_debug("No token info in getTrackStaticData"))
        complete_source_code_link = ""
        complete_source_code_found = 0
    ...
    # Use our safe function to get current playback
    current_track = safe_get_current_playback()
    
    if current_track is None:
        print(format_debug("No current track found in getTrackStaticData"))
        complete_source_code_link = ""
        complete_source_code_found = 0
    ...
    # Check if item exists and contains necessary data
    if 'item' not in current_track or current_track['item'] is None:
        print(format_debug("Item is missing or None in current_track"))
        complete_source_code_link = ""
        complete_source_code_found = 0
    ...
    try:
        ...
        if 'artists' in current_track['item'] and current_track['item']['artists'] and len(current_track['item']['artists']) > 0:
            artist_name = current_track['item']['artists'][0]['name']
        else:
            artist_name = "Unknown Artist"
            
        # Get track duration
        track_duration_ms = current_track['item'].get('duration_ms', 0)
        minutes, seconds = divmod(track_duration_ms / 1000, 60)
        
        # Get album name for better LRCLIB matching
        album_name = None
        if 'album' in current_track['item'] and current_track['item']['album'] is not None:
            album_name = current_track['item']['album'].get('name')
        
        # Safely extract album cover URL
        album_cover_url = ""
        if 'album' in current_track['item'] and current_track['item']['album'] is not None:
            if 'images' in current_track['item']['album'] and current_track['item']['album']['images']:
                album_cover_url = current_track['item']['album']['images'][0]['url']
        
        print(format_debug(f"Successfully got basic track info: {track_name} by {artist_name}"))
        ...
        if complete_source_code_found == 1:
            ...
            # Pass album_name and track_duration to improve LRCLIB matching
            synced_lyrics_json, found_musixmatch_lyrics, musixmatch_lyrics_is_linesynced = getSyncedLyricsJson(
                track_id, artist_name, track_name, album_name, track_duration_ms
            )
            ...
            if (found_musixmatch_lyrics == 1 and musixmatch_lyrics_is_linesynced == 1):
                # Adjust timestamps based on which provider was used
                if 'lrclib_lyrics_provider' in globals() and track_duration_ms > 0:
                    synced_lyrics_json = lrclib_lyrics_provider.adjust_timestamps(synced_lyrics_json, track_duration_ms)
                elif 'llm_lyrics_provider' in globals() and track_duration_ms > 0:
                    synced_lyrics_json = llm_lyrics_provider.adjust_timestamps(synced_lyrics_json, track_duration_ms)
                ...
                if error_syncing:
                    print(format_debug(f"Sync error: {error_syncing}"))
                    sync_ratio_percentage = "0"
                    found_musixmatch_lyrics = 0
                    musixmatch_lyrics_is_linesynced = 0
                    return {
    ...
    # Handle any exceptions in the function
    except Exception as e:
        print(format_debug(f"Error in getTrackStaticData: {e}"))
        complete_source_code_link = ""
        complete_source_code_found = 0
        ...
        main_chords_body  = f"Error processing track data: {str(e)}"
    ...
    if cached_data:
        ...
        if (current_date - save_timestamp).days > days_to_renew:
            try:
                ...
                # Log Ultimate Guitar result
                log_ultimate_guitar_tab(track_id, track_name, artist_name, complete_source_code, 
                                         complete_source_code_link, complete_source_code_found == 1)
...
def getSyncedLyricsJson(track_id, artist_name, track_name, album_name=None, track_duration_ms=None):
    ...
    if cached_data:
        ...
        if (current_date - save_timestamp).days > days_to_renew:
            try:
                # First try LRCLIB
                print(format_debug("Fetching lyrics using LRCLIB..."))
                original_json = lrclib_lyrics_provider.get_lyrics(track_id, artist_name, track_name, album_name, track_duration_ms)
                
                # Log LRCLIB result
                log_lrclib_lyrics(track_id, track_name, artist_name, original_json)
    ...
    try:
        # Create a response format similar to what the app expects
